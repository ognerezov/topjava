...
public class MealRestControllerTest extends AbstractControllerTest {
    ...
    public void testDelete() throws Exception {
        mockMvc.perform(delete(REST_URL + MEAL1_ID).with(userHttpBasic(USER)).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    ...
    public void testUpdate() throws Exception {
        ...
        mockMvc.perform(put(REST_URL + MEAL1_ID).with(userHttpBasic(USER)).contentType(MediaType.APPLICATION_JSON)
    ...
    public void testGetAll() throws Exception {
        mockMvc.perform(get(REST_URL).with(userHttpBasic(USER)).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
    ...
    public void testGetBetween() throws Exception {
        mockMvc.perform(get(REST_URL + "between?startDateTime=2015-05-30T07:00&endDateTime=2015-05-31T11:00:00").with(userHttpBasic(USER)))
                .andExpect(status().isOk())
                .andDo(print())
    ...
    public void testFilter() throws Exception {
        mockMvc.perform(get(REST_URL + "filter").with(userHttpBasic(USER))
